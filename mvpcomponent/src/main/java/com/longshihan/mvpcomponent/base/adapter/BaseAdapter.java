...
public abstract class BaseAdapter<T> extends RecyclerView.Adapter<BaseHolder<T>> {
    ...
    public void replaceList(List<T> datas) {
        if (mInfos == null) {
    ...
    public void appendList(List<T> mInfoLists) {
        if (mInfos == null) {
            mInfos = new ArrayList<>();
        }
        if (mInfoLists == null) {
        ...
        int currentIndex=mInfos.size();
        if (mInfos.addAll(mInfoLists)){
            notifyItemRangeInserted(currentIndex-1,mInfoLists.size());
        }
    }
