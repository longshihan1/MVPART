package com.longshihan.baseadapter.loadmore;
...
public abstract class LoadMoreView {
    ...
    private void visibleLoading(BaseViewHolder holder, boolean visible) {
        holder.getView(getLoadingViewId()).setVisibility(visible? View.VISIBLE:View.GONE);
    }

    private void visibleLoadFail(BaseViewHolder holder, boolean visible) {
        holder.getView(getLoadFailViewId()).setVisibility(visible? View.VISIBLE:View.GONE);
    }

    private void visibleLoadEnd(BaseViewHolder holder, boolean visible) {
        final int loadEndViewId = getLoadEndViewId();
        if (loadEndViewId != 0) {
            holder.getView(loadEndViewId).setVisibility(visible? View.VISIBLE:View.GONE);
        }
    }
